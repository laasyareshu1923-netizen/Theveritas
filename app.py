...
# 3. Live Web Verification Tool Function with Full Error Reporting
def fetch_real_web_data(query_text):
    try:
        ...
        # High-end desktop headers to bypass basic anti-bot blocks
        headers = {
        ...
        with urllib.request.urlopen(req, timeout=7) as response:
            ...
            for item in root.findall('.//item')[:3]:
                ...
                source_name = item.find('source').text if item.find('source') is not None else "Verified News"
            ...
            return discovered_articles, None
    except Exception as e:
        # Instead of returning empty, return the real error text to print on screen
        return [], str(e)
...
with col_input:
    search_term = st.text_input(
        ...
        placeholder="Type a claim to test...",
...
if go_clicked and search_term:
    with st.spinner("Processing neural layers..."):
        ai_prediction = ai_engine(search_term)[0]
        st.session_state.current_label = ai_prediction['label']
        st.session_state.current_score = ai_prediction['score'] * 100
...
if st.session_state.ai_analyzed:
    ...
    if opt_factcheck:
        with st.spinner("Scanning live regional news registries..."):
            live_news_results, error_msg = fetch_real_web_data(active_text)
            
            if error_msg:
                # Direct transparent disclosure of the network error
                st.error(f"📡 Cloud Server Network Blocked: {error_msg}")
                st.info("💡 Note: Streamlit's cloud hosting network is currently being rate-limited or blocked by Google's anti-bot firewall. The code structure is fine, but the hosting server IP is restricted.")
            elif live_news_results:
                st.success("Real Matches Found in Live Broadcast Logs")
                ...
                st.warning("Zero Verified Media Matches Discovered on Current Channels.")

    elif opt_bias:
        bias_analysis = "This phrasing relies on extreme buzzwords, intense punctuation, or urgency framing." if label == "NEGATIVE" else "Linguistic structures are descriptive, objective, and flat."
